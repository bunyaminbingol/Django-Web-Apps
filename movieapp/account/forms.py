...
class UserPasswordChangeForm(PasswordChangeForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['old_password'].widget = widgets.PasswordInput(attrs={'class':'form-control', 'placeholder':'Eski şifrenizi giriniz.'})
        self.fields['new_password1'].widget = widgets.PasswordInput(attrs={'class':'form-control', 'placeholder':'Yeni şifrenizi giriniz.'})
        self.fields['new_password2'].widget = widgets.PasswordInput(attrs={'class':'form-control', 'placeholder':'Yeni şifrenizi tekrar giriniz.'})

class LoginForm(forms.Form):
    email = forms.EmailField(widget=forms.EmailInput(attrs={'class': 'form-control form-control-user', 'placeholder': 'Enter Email Adress...'}))
    ...
    remember_me = forms.BooleanField(required=False, initial=False, widget=forms.CheckboxInput(attrs={
        "class":"custom-control-input"})) # initial=False başlangıçta checkboxının kontrolünü kaldırıyoruz.
...
class CreateUserForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["email"].required = True # email alanı zorunlu olsun diye.
    ...
    def save(self, commit=True): # commit=True ise kayıt yapılır.
        user = super(UserCreationForm, self).save(commit=False) # super ile UserCreationForm'ın save metodunu çağırıyoruz.
        user.set_password(self.cleaned_data["password1"]) # password1'i set ediyoruz.
